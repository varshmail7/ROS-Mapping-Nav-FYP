...
class TurtleBot:
    ...
    def calc_error(self,data):
        ...
        self.error = sqrt(pow((self.goal_pose.x - self.pose.x), 2) +
                    pow((self.goal_pose.y - self.pose.y), 2)) - sqrt(pow((self.goal_pose.x - data.pose.pose.position.x), 2) +
                                pow((self.goal_pose.y - data.pose.pose.position.y), 2))
    ...
    def linear_vel(self, goal_pose, kp=0.2, kd=5, ki=0.1):
    ...
    def angular_vel(self, goal_pose, aoa =0, kp=1.5 ,kd=0.01):
        ...
        if aoa==0:
            ...
            return 1 *kp * (goal_pose.theta - self.pose.theta) + kd*self.thetaerror


    def move2goal(self,x,y,angleOfApproach):
        ...
        while abs(self.steering_angle(goal_pose,1)) >= angle_tolerance:
            vel_msg.linear.x = 0
            vel_msg.linear.y = 0
            vel_msg.linear.z = 0
